...
const GroupEditForm = ({ initialData = {}, onComplete }) => {
  ...
  const [previewUrl, setPreviewUrl] = useState(initialData.profile_picture || null);
    ...
    <form onSubmit={handleSubmit} className="space-y-2 max-h-[80vh] overflow-y-auto px-1">
      ...
      <div className="mb-2">
        ...
        <div className="flex items-center space-x-2">
          <div className={`h-12 w-12 rounded-full overflow-hidden border ${isDarkTheme ? "border-gray-700" : "border-gray-200"}`}>
            {previewUrl ? (
              <img 
                src={previewUrl.startsWith && previewUrl.startsWith("http") ? previewUrl : (profilePicture ? URL.createObjectURL(profilePicture) : previewUrl)} 
                alt="Aperçu" 
                className="h-full w-full object-cover"
              />
            ) : (
              <div className={`h-full w-full flex items-center justify-center ${isDarkTheme ? "bg-gray-700" : "bg-gray-200"}`}>
                {formData.nom ? formData.nom.substring(0, 2).toUpperCase() : "GP"}
              </div>
            )}
          </div>
          ...
          <div className="flex-1">
            <input
              ...
              className={`w-full text-xs ${isDarkTheme ? "text-gray-300" : "text-gray-700"}`}
            />
      ...
      <div className={`flex items-center justify-between border p-2 rounded-md text-sm ${
...
const Group = () => {
  ...
  const fetchGroupData = async () => {
    ...
    try {
      ...
      // Use members field from updated API response
              ...
              <div className="absolute -bottom-16 left-6">
                <Avatar className="h-32 w-32 border-4 border-white shadow-md">
                  {group?.profile_picture ? (
                    <AvatarImage src={group.profile_picture.startsWith("http")?group.profile_picture : `http://localhost:8000${group.profile_picture}`} alt="Group Logo" />
                  ) : null}
                  <AvatarFallback
                    className={`text-3xl ${
